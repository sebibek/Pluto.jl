...
`,M=[{title:"Notebooks",tags:"everything"}],U=e=>(null==e?void 0:e.id)??e.url,F=({sources:e,direct_html_links:t})=>{const[o,n]=r.useState({});r.useEffect((()=>{if(null!=e){a(!1),n({});const t=Array.from(new Set(e.map(U))).map((t=>{const o=e.filter((e=>U(e)===t));return I(o.map((async e=>{const{url:o,integrity:n,valid_until:r}=e;if(null!=r&&new Date(r)<new Date)throw new Error(`Source ${o} is expired with valid_until ${r}`);const l=await(await fetch(new Request(o,{integrity:n??void 0}))).json();if("1"!==l.format_version)throw new Error(`Invalid format version: ${l.format_version}`);return[l,t,o]}))).then((([e,t,o])=>{n((n=>({...n,[t]:{...e,source_url:o}})))}))}));Promise.any(t).catch((e=>{console.error("All featured sources failed to load: ",e),((null==e?void 0:e.errors)??[]).forEach((e=>console.error(e))),a(!0)}))}}),[e]),r.useEffect((()=>{Object.entries(o).length>0&&console.log("Sources:",o)}),[o]);const[l,a]=r.useState(!1);r.useEffect((()=>{setTimeout((()=>{a(!0)}),8e3)}),[]);const i=0===Object.entries(o).length,s=Array.from(new Set((null==e?void 0:e.map(U))??[])).map((e=>o[e])).filter((e=>null!=e));return i&&l?D:r.html`
